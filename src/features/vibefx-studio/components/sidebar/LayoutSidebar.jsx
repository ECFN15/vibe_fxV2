        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 animate-in slide-in-from-right-4">
            ...
            <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                    {FORMATS.map(fmt => (
                        <button key={fmt.id} onClick={() => setActiveFormat(fmt)} className={`flex-1 min-w-[70px] flex flex-col items-center justify-center p-3 border transition-all ${activeFormat.id === fmt.id ? 'bg-indigo-900/30 border-indigo-500 text-indigo-400' : (isDarkMode ? 'border-neutral-800 bg-transparent text-neutral-500 hover:border-neutral-600' : 'border-gray-200 bg-gray-50 text-gray-500 hover:bg-white')}`}>
                            ...
                            <span className="text-[10px] font-mono font-bold uppercase" translate="no">{fmt.label.replace(/\s*\(.*?\)/, '')}</span>
                        </button>
                    ))}
                </div>
            </div>
