...
export default function SmoothBlurPopup({ images, isOpen, onClose, isDarkMode, initialConfig, onApply }) {
    ...
    return createPortal(
        <div className="vibefx-smoothblur-layer fixed inset-0 z-[999999] flex items-center justify-center p-4 sm:p-8" style={{ pointerEvents: 'auto' }}>
            {/* 1. Click Catcher */}
            ...
            <div className={`vibefx-smoothblur-modal relative w-[80vw] max-w-[1200px] h-[80vh] min-h-[600px] max-h-[900px] flex overflow-hidden rounded-[24px] border ${borderColor} shadow-[0_0_120px_rgba(0,0,0,0.8)] ${bgPanel} animate-in zoom-in-95 duration-300`}>

                {/* Left Sidebar: Controls */}
                <div className={`vibefx-smoothblur-controls w-[340px] h-full pt-8 pb-6 px-6 border-r ${borderColor} ${bgPanel} flex flex-col gap-2 overflow-y-auto custom-scrollbar z-0 text-[#e5e5e5]`}>
                ...
                <div className="vibefx-smoothblur-preview flex-1 flex flex-col relative" style={{ backgroundImage: 'radial-gradient(circle, #262626 1px, transparent 1px)', backgroundSize: '16px 16px', backgroundPosition: 'center', backgroundColor: isDarkMode ? '#050505' : '#f5f5f5' }}>

                    {/* Close button on Top Right */}
                    <div className="vibefx-smoothblur-close absolute top-6 right-6 z-20">
                        <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-colors text-neutral-400 hover:text-white">
                            <X size={20} />
                        </button>
                    </div>

                    <div className="vibefx-smoothblur-stage flex-1 flex items-center justify-center p-12 overflow-hidden relative">
                        <div className={`vibefx-smoothblur-frame relative transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] shadow-2xl rounded-sm overflow-hidden border ${borderColor}`} style={{ width: '100%', maxWidth: '440px', aspectRatio: '4/5' }}>
                    ...
                    {/* Bottom Export Bar */}
                    <div className={`vibefx-smoothblur-footer h-20 border-t ${borderColor} px-8 flex items-center justify-between bg-black/40 backdrop-blur-md`}>
