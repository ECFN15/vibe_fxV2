...
|   |   |   |-- music/
|   |   |   |   |-- free-search/
|   |   |   |   |   `-- route.js        # Agregateur serveur sources gratuites : Pixabay manuel, Openverse, Jamendo, Freesound, Archive, Wikimedia
|   |   |   |   |-- import/
|   |   |   |   |   `-- route.js        # Proxy serveur controle pour URL audio directe allowlistee, validation host final, sans scraping de catalogue
|   |   |   |   `-- providers/
|   |   |   |       `-- route.js        # Metadata providers musique, statuts cle manquante et limite officielle Pixabay audio
|   |   |   |-- proxy-image/
...
|   |   |-- soundtrack/                 # Onglet Soundtrack full page V2 : bibliotheque projet Firebase owner-scoped + playlists projet + agregateur sources gratuites + legacy local-first
|   |   |   |-- components/               # ProjectLibraryPanel avec playlists/classement projet, AggregatorPanel, Search, results/rows/player, playlists locales, dossier local et droits/licences
...
|   |-- smoke-soundtrack-core.mjs       # Smoke pur Soundtrack V2 : mapping ProviderTrack -> ProjectSoundTrack, droits, cache key provider, manifest sans Blob/File
|   |-- smoke-soundtrack-ui.spec.cjs    # Smoke Playwright onglet Soundtrack V2 : scan mocke, favoris/playlists locaux, manifest, providers/API, responsive
...
- `/api/music/free-search` : API interne d'agregation officielle pour sources gratuites Pixabay manuel/Openverse/Jamendo/Freesound/Archive/Wikimedia, sans cle provider exposee au client, avec scans volontaires et cache controle.
...
- `/api/music/providers` : API interne de metadata providers musique ; expose la limite verifiee Pixabay (pas d'endpoint audio public documente) et les statuts `provider-missing-key`.
