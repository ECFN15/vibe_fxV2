...
|   |-- smoke-soundtrack-ui.spec.cjs    # Smoke Playwright onglet Soundtrack V2 : provider-first Openverse mocke, aucun fallback Vibe_CUT dans le scan, favoris/playlists locaux, manifest, providers/API, responsive
...
- L'onglet `Soundtrack` est ajoute au header studio apres `Library` et ouvre une page full screen sans canvas/sidebar. La feature `src/features/vibefx-studio/soundtrack/` reutilise `/api/music/free-search`, `/api/music/import`, `/api/music/project/import-url`, `/api/music/providers`, `musicCatalog.js`, `musicRights.js` et `audioWaveform.js`; elle separe maintenant une bibliotheque projet Vibe_fx owner-scoped (`users/{uid}/soundtrackTracks/{trackId}` + Storage `users/{uid}/soundtrack/{trackId}/{fileName}`) de l'agregateur provider-first. L'agregateur affiche une barre providers, utilise Openverse Audio comme provider actif pour le scan par categories/tags, garde Pixabay Music visible mais desactive avec statut page-scan bloque, masque les filtres generiques Licence/BPM/genre/mood/duree/pages et n'injecte plus les pistes locales Vibe_CUT comme fallback de resultats. Si un provider renvoie une erreur, l'UI affiche l'indisponibilite et reste vide cote scan au lieu d'afficher de faux resultats; les imports manuels URL audio directe restent disponibles dans le panneau local. Les favoris/playlists/manifest locaux restent disponibles via IndexedDB + `vibefx-soundtrack.json` et File System Access/fallback download. L'action `Vibe_CUT` transmet un `Blob/File` local ou un blob recupere depuis la piste projet avec manifeste droits.
