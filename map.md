...
|   |   |-- components/                 # Header, tabs Studio/Fusion/Layout/Library/Soundtrack/Credits/Vision/Video et panneaux source Vibe_fx
...
|   |   |-- soundtrack/                 # Onglet Soundtrack full page local-first : recherche multi-sources, favoris/playlists IndexedDB, manifest JSON, File System Access/fallback download, droits et envoi local vers Vibe_CUT
|   |   |   |-- components/               # Search, results/rows/player, playlists, dossier local et droits/licences
|   |   |   |-- data/                     # Providers/filtres/defaults Soundtrack reutilisant musicCatalog
|   |   |   |-- hooks/                    # Recherche API, player preview et bibliotheque locale IndexedDB/dossier
|   |   |   |-- services/                 # IndexedDB, manifest, File System Access, downloads locaux et audit droits
...
|   |-- smoke-soundtrack-ui.spec.cjs    # Smoke Playwright onglet Soundtrack local-first : recherche mockee, favoris/playlists IndexedDB, manifest, responsive, refus stockage serveur
...
- L'onglet `Soundtrack` est ajoute au header studio apres `Library` et ouvre une page full screen sans canvas/sidebar. La feature `src/features/vibefx-studio/soundtrack/` reutilise `/api/music/free-search`, `/api/music/import`, `musicCatalog.js`, `musicRights.js` et `audioWaveform.js`; elle garde favoris/playlists/manifest en local via IndexedDB + `vibefx-soundtrack.json`, ecrit dans un dossier local via File System Access quand disponible, bascule sinon en telechargement/import manuel, et n'envoie jamais les fichiers audio utilisateur vers Firebase/Firestore/Functions/Storage. L'action `Vibe_CUT` transmet un `Blob/File` local au store video avec manifeste droits.
