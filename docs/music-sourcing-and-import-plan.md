...
- Pixabay was rechecked against the official API documentation on 2026-05-21: the public API documentation exposes image and video search endpoints, not a documented public audio search/download endpoint. Soundtrack therefore keeps Pixabay as `manual-url-only`: no catalog scraping, no automatic mass scan, and only controlled direct audio URL import when the user supplies an allowed URL plus source/license metadata.
- `src/app/api/music/providers/route.js` exposes provider capability metadata to the app, including missing-key states for Jamendo/Freesound and the validated Pixabay limitation.
