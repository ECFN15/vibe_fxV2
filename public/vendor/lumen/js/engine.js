...
var Engine = (function () {
  ...
  function setSize(w, h) {
    w = Math.max(2, Math.round(w || 0));
    h = Math.max(2, Math.round(h || 0));
    if (!canvas || !gl) return;
  ...
  function renderAt(phase) {
    if (!canvas || !gl || canvas.width < 2 || canvas.height < 2) return;
    var P = getParams();
  ...
  function tick(now) {
    ...
    fpsAcc += dt; fpsN++;
    if (fpsAcc >= 0.5) {
      fps = Math.min(30, Math.round(fpsN / fpsAcc));
      fpsAcc = 0; fpsN = 0;
      if (fpsCb) fpsCb(fps);
    }
    requestAnimationFrame(tick);
