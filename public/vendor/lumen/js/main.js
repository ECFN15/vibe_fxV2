...
var ASPECTS = { "16:9": 16 / 9, "9:16": 9 / 16, "3:2": 1.5, "1:1": 1, "4:5": 0.8, "21:9": 21 / 9 };
...
var P = {
  ...
  lockStyle: false,
  imgRes: "1080", vidRes: "720", vidFps: "30", vidLen: "l1",
...
function fitCanvas() {
  ...
  if (!frame || !window.Engine || !Engine.canvas) return false;

  var rect = frame.getBoundingClientRect();
  var frameW = frame.clientWidth || rect.width || 0;
  var frameH = frame.clientHeight || rect.height || 0;

  if (frameW < 120 || frameH < 120) {
    var app = document.querySelector(".app");
    var rail = document.getElementById("rail");
    var appRect = app ? app.getBoundingClientRect() : null;
    var railW = rail ? Math.min(360, rail.getBoundingClientRect().width || 0) : 0;
    frameW = Math.max(frameW, (appRect && appRect.width ? appRect.width - railW : 0), window.innerWidth - railW, 480);
    frameH = Math.max(frameH, (appRect && appRect.height ? appRect.height : 0), window.innerHeight - 150, 320);
  }

  var availW = Math.max(240, frameW - 80);
  var availH = Math.max(180, frameH - 52);
  var ar = ASPECTS[P.aspect] || (16 / 9);
  var w = availW, h = w / ar;
...
function fitCanvasWhenReady(attempt) {
  attempt = attempt || 0;
  var ok = fitCanvas();
  if (!ok && attempt < 24) {
    window.setTimeout(function () { fitCanvasWhenReady(attempt + 1); }, attempt < 6 ? 50 : 150);
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fitCanvasWhenReady();
  window.addEventListener("load", function () { fitCanvasWhenReady(); });
  window.addEventListener("resize", function () { fitCanvasWhenReady(); });
  document.addEventListener("visibilitychange", function () {
    if (document.hidden) Engine.suspend();
    else { Engine.resume(); fitCanvasWhenReady(); }
  });
  new ResizeObserver(function () { fitCanvasWhenReady(); }).observe(document.getElementById("canvas-frame"));
