...
const appCheckEnforced = values.ENFORCE_META_APP_CHECK === "true" || process.env.ENFORCE_META_APP_CHECK === "true";
...
if (!missingClientEnv.length && !missingMetaSecrets.length && !missingAppCheckSiteKey) {
  ...
  console.log("- If ENFORCE_META_APP_CHECK=true, Firebase App Check is enabled for the app and callable Functions.");
...
if (missingAppCheckSiteKey) {
  console.log("\nMissing App Check client env while ENFORCE_META_APP_CHECK=true:");
...
console.log("- If ENFORCE_META_APP_CHECK=true, enable Firebase App Check for the web app before OAuth publication.");
